...
class ClaimApiKeys(Resource):
    ...
    def post(self):
        ...
        if (not self.ContributorsDb.IsContributor(claimer)):
            return self.__ErrorManager.BadRequestError("You are not a contributor").ToDict(), 400

        if (self.ApiKeyLogDb.isApiKeyForContributor(claimer, request.remote_addr)):
            return self.__ErrorManager.BadRequestError("You already have an apiKey").ToDict(), 400
