...
func (wille *Wille) uploadWhitelistFile(name string) error {
	err, inOut, inErr := wille.mongoImport(DEV_URI_USERS_DB, "Whitelist", "Data/"+name+"/List/Whitelist.json")
...
func (wille *Wille) checkWhitelistJsonContent(name string) error {
	jsonFile, err := os.Open("Data/" + name + "/List/Whitelist.json")
	...
	if err != nil {
		return errors.New("Problem with json file " + name + "/List/Whitelist.json" + ": " + err.Error())
	}
	return nil
}
